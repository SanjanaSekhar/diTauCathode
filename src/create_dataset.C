...
int create_dataset(string file_n, int label) {
	...
	// QCD xsec : 437700 pb
	// Wjets xsec : 1.629771e+04
	float xsec = 1.629771e+04;
	if(isSig) xsec = -1; 
	...
	// sum of weights = 1.63802e+10 for Wjets
	float sum_weights = 1.63802e+10; 
	...
	for (Long64_t entry = 0; entry < numberOfEntries; ++entry) {
		...
		if(entry%20000 == 0) std::cout << "Calculating weights sum for entry = " << entry << "\n";
	}
